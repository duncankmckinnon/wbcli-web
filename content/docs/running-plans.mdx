...
This looks up the session in `.workbench/status-*.yaml`, finds the original plan, and re-runs every task that isn't `done + merged` — including tasks that failed and tasks that never started. For finer control (wave ranges, directive overrides, selective tasks), use `wb run` directly:
