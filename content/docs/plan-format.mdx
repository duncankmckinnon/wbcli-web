...
>
> - **`wb plan "<prompt>"`** — dispatches a planner agent from the CLI. Generates a plan at `.workbench/plans/plan.md`. Can also transform an existing document with `--from`. See [CLI Reference](/docs/cli-reference#wb-plan).
