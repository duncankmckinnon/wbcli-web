...
Plans can declare run-time defaults — session branch, base, concurrency, TDD, final review, and more — in a YAML frontmatter block at the very top of the file, before the `# Title` line. CLI flags always override frontmatter, which overrides built-in defaults.

See **[Plan Frontmatter](/docs/plan-frontmatter)** for the full schema, precedence rules, and examples.
