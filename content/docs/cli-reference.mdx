...
| `--trace-env/--no-trace-env` | Inject `WB_*` trace env vars into agent processes. Default: on. |
...
| `--trace/--no-trace` | Master switch for session trace metadata (env + prompt). Default: on. |
| `--trace-env/--no-trace-env` | Inject `WB_*` trace env vars into the planner agent process. Default: on. |
