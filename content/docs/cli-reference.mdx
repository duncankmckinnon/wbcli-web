...
The `summarizer` and `branch_reviewer` roles are configurable in `profile.yaml` like any other role (`agent`, `directive`). See [Profiles](/docs/profiles) and [Review & Pull Requests](/docs/review-pr-flow).
