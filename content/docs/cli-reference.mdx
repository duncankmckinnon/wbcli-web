...
| `wb clean [project]` | Remove workbench worktrees, branches, and status artifacts; `--purge` deletes entire plan directories |
...
| `--trace/--no-trace` | Master switch for session trace metadata (env + prompt). Default: on. |
| `--trace-env/--no-trace-env` | Inject `WB_*`/OTEL trace env vars into agent processes. Default: on. |
| `--trace-prompt/--no-trace-prompt` | Prepend a `wb-session` metadata block to agent prompts. Default: off. |
...
| `trace` | `--trace/--no-trace` | bool |
| `trace_env` | `--trace-env/--no-trace-env` | bool |
| `trace_prompt` | `--trace-prompt/--no-trace-prompt` | bool |
...
| `--trace/--no-trace` | Master switch for session trace metadata (env + prompt). Default: on. |
| `--trace-env/--no-trace-env` | Inject `WB_*`/OTEL trace env vars into the planner agent process. Default: on. |
| `--trace-prompt/--no-trace-prompt` | Prepend a `wb-session` metadata block to the planner agent's prompt. Default: off. |
| `--repo PATH` | Repository path (auto-detected if omitted) |
...
Remove workbench-created git worktrees, `wb/` branches, and status artifacts. By default, only removes artifacts for plans where every task is done and merged. Pass `--force` to wipe everything, or `--purge` to delete entire plan directories.

```bash
wb clean [PROJECT] [options]
```

`PROJECT` is the plan folder name under `.workbench/` (e.g. `wb clean my-plan`) or a path to the plan file (e.g. `wb clean .workbench/my-plan/plan.md`). When omitted, all plans are considered.

### Flags

| Flag | Description |
|---|---|
| `--force` | Remove all worktrees and `wb/` branches, including in-flight ones |
...
| `--purge` | Delete the entire `.workbench/<project>/` directory tree (or all plan subdirectories when no PROJECT is given), plus all associated git artifacts. Preserves root-level config files (`conventions.md`, `agents.yaml`, `profile.yaml`). Implies `--force`. |
| `--repo PATH` | Repository path (auto-detected if omitted) |

`--purge` and `--force` are mutually exclusive (purge implies force). `--purge` and `--completed` are also mutually exclusive.

### Examples

```bash
wb clean                          # remove artifacts for all completed plans
wb clean my-plan                  # scope cleanup to a single plan
wb clean --force                  # wipe all worktrees and branches regardless of status
wb clean --completed              # remove completed plans, silently skip in-flight ones
wb clean --purge                  # delete all plan directories (preserves config files)
wb clean my-plan --purge          # delete the entire .workbench/my-plan/ directory
wb clean --purge --dry-run        # preview what --purge would delete
wb clean --remove-plans           # also delete plan.md files for completed plans
```
...
Create `agents.yaml` pre-populated with configs for all built-in adapters (Claude, Claude YOLO, Antigravity, OpenCode, Kimi, Codex, Cursor, Copilot). Use as a starting point for customization.
...
```bash
wb profile set implementor.agent antigravity
