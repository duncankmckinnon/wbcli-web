...
[thirdeye](https://thrdi.com) records Claude Code and Codex sessions locally. Workbench adds structured `WB_*` environment variables to every agent it launches; thirdeye can turn those values into tags automatically. The result is a trace inventory you can filter by plan, wave, task, role, or exact pipeline step.
