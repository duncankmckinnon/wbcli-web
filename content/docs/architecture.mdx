...
`.workbench/agents.yaml` — registers custom agent adapters so they can be used in profiles and `--agent` flags. Built-in adapters (Claude, Antigravity, Codex, Cursor, Copilot) are available without any config. See [Agents](/docs/agents#custom-agents).
