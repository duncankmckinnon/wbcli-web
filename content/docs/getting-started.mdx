...
- **At least one supported agent CLI:**
  ...
  - [Google Antigravity](https://github.com/google/agy) — `agy` command
  - [OpenCode](https://opencode.ai) — `opencode` command
  - [Kimi Code](https://www.kimi.com/code/docs/en/kimi-code-cli/) — `kimi` command
  - [OpenAI Codex](https://github.com/openai/codex) — `codex` command
  - [Cursor CLI](https://cursor.com/docs/cli/overview) — `agent` command
...
> **Tip:** Installing via Homebrew automatically pulls in Python, git, and tmux. The manual prerequisite install above is mainly for the pip and from-source paths.
...
This taps `duncankmckinnon/tap` and installs the `wb` command along with its dependencies — Python 3.12, git, and tmux.

### pip / uv / pipx

Install from PyPI into your active Python environment:

```bash
pip install wbcli       # or: uv tool install wbcli
                        # or: pipx install wbcli
```
...
The `wb` command is now available on your PATH. Requirements: Python 3.11+ and git on PATH; tmux is optional — run with `--no-tmux` if you don't have it.
...
> **Don't want to write the plan yourself?** Run `wb plan "describe what you want to build"` to have a planner agent survey your codebase and produce a ready-to-run plan at `.workbench/plan/plan.md`. You can also convert an existing spec with `wb plan --from spec.md`. See [CLI Reference](/docs/cli-reference#wb-plan) for full options.
...
> **Plan frontmatter (optional):** Plans can declare run-time defaults in a YAML frontmatter block at the very top of the file, before `# Title`. CLI flags always override frontmatter values. See [Plan Frontmatter](/docs/plan-frontmatter) for the full schema and examples.
>
> ````markdown
> ---
...
- **[Agents](/docs/agents)** — configure multi-provider setups with Claude, Antigravity, OpenCode, Kimi, Codex, Cursor, and Copilot
- **[Running Plans](/docs/running-plans)** — monitoring, failure handling, merging, and cleanup
- **[thirdeye Integration](/docs/thirdeye)** — tag traces by plan, wave, task, role, and pipeline step
