...
Workbench uses 9 directive variants in total:
...
All 9 support both `directive` (full replace) and `directive_extend` (append to default) at their respective YAML paths. CLI flags (`--*-directive`) are limited to the four main roles (`implementor`, `tester`, `reviewer`, `fixer`) and always do a full replacement for that run.
